...
import {AnyWrapperClass, CommonWrapper, Constructor} from 'mathjax-full/js/output/common/Wrapper';
import {JSONOutput, XLINKNS} from '../json';
import {JSONWrapperFactory} from './WrapperFactory';
import {JSONCharOptions, JSONDelimiterData, JSONFontData} from './FontData';

export type {Constructor, StringMap} from 'mathjax-full/js/output/common/Wrapper';

/*****************************************************************/

/**
 * Shorthand for makeing a JSONWrapper constructor
 */
export type JSONConstructor<N, T, D> = Constructor<JSONWrapper<N, T, D>>;


/*****************************************************************/
/**
 *  The type of the JSONWrapper class (used when creating the wrapper factory for this class)
 */
export interface JSONWrapperClass extends AnyWrapperClass {
...
/*****************************************************************/
/**
 *  The base JSONWrapper class
 *
 * @template N  The HTMLElement node class
 * @template T  The Text node class
 * @template D  The Document class
 */
export class JSONWrapper<N, T, D> extends CommonWrapper<
    JSONOutput<N, T, D>,
    JSONWrapper<N, T, D>,
    JSONWrapperClass,
    JSONCharOptions,
...
> {
     ...
     * @override
     */
    public parent: JSONWrapper<N, T, D>;
    /**
     * @override
     */
    public childNodes: JSONWrapper<N, T, D>[];
    /**
     * The JSON element generated for this wrapped node
     */
     ...
     * @override
     */
    public font: JSONFontData;
    /**
     * The factory used to create more JSONWrappers
     */
    protected factory: JSONWrapperFactory<N, T, D>;
    ...
    public toJSON(parent: N) {
        this.addChildren(this.standardJSONnode(parent));
    }
     ...
     * @param {OptionList} def   The properties to set for the created node
     * @param {(N|T)[]} content  The child nodes for the created JSON node
     * @return {N}               The generated JSON tree
     */
    ...
    /**
     * Create the standard JSON element for the given wrapped node.
     *
     ...
     */
    protected standardJSONnode(parent: N): N {
        const svg = this.createJSONnode(parent);
        this.handleStyles();
        this.handleScale();
        this.handleBorder();
        this.handleColor();
     ...
     * @returns {N}  The root of the HTML tree for the wrapped node's output
     */
    protected createJSONnode(parent: N): N {
        const text = this.getText();
        const def = {
            'data-mml-node': this.node.kind,
        };
        if (!!text) {
            def['data-mml-text'] = text;
        }
        this.element = this.svg('g', def);
    ...
    protected handleBorder() {
        ...
        const f = JSONWrapper.borderFuzz;
    ...
    /**
     * Copy RDFa, aria, and other tags from the MathML to the JSON output nodes.
    ...
    protected handleAttributes() {
        ...
        const skip = JSONWrapper.skipAttributes;
