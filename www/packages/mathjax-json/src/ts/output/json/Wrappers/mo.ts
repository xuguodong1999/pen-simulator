...
import {CommonMoMixin} from 'mathjax-full/js/output/common/Wrappers/mo';
import {MmlMo} from 'mathjax-full/js/core/MmlTree/MmlNodes/mo';
import {BBox} from 'mathjax-full/js/util/BBox';
import {JSONCharData, JSONCharOptions} from '../FontData';

// copy from mathjax-full/ts/output/common/FontData.ts, fix const enum undefined failure
const enum DIRECTION {None, Vertical, Horizontal}
...
/*****************************************************************/
/**
 * The JSONmo wrapper for the MmlMo object
 *
 ...
 */
// @ts-ignore
export class JSONmo<N, T, D> extends CommonMoMixin<JSONConstructor<any, any, any>>(JSONWrapper) {
    ...
    /**
     * Create the JSON for a multi-character stretchy delimiter
     */
    protected stretchJSON() {
     ...
     * @return {JSONCharData}     The full CharData object, with CharOptions guaranteed to be defined
     */
    protected getChar(n: number, variant: string): JSONCharData {
        const char = this.font.getChar(variant, n) || [0, 0, 0, null];
        return [char[0], char[1], char[2], char[3] || {}] as [number, number, number, JSONCharOptions];
    }
