import {CommonOutputJax, UnknownBBox} from 'mathjax-full/js/output/common/OutputJax';
import {OptionList} from 'mathjax-full/js/util/Options';
import {MathDocument} from 'mathjax-full/js/core/MathDocument';
import {MathItem} from 'mathjax-full/js/core/MathItem';
import {MmlNode} from 'mathjax-full/js/core/MmlTree/MmlNode';
import {JSONWrapper} from './json/Wrapper';
import {JSONWrapperFactory} from './json/WrapperFactory';
import {JSONFontData} from './json/FontData';
import {TeXFont} from './json/fonts/tex';
import {StyleList as CssStyleList} from 'mathjax-full/js/util/StyleList';
import {FontCache} from './json/FontCache';
import {unicodeChars} from 'mathjax-full/js/util/string';
import {percent} from 'mathjax-full/js/util/lengths';
 ...
 */
export class JSONOutput<N, T, D> extends CommonOutputJax<N, T, D, JSONWrapper<N, T, D>, JSONWrapperFactory<N, T, D>, JSONFontData, typeof JSONFontData> {

    /**
     * The name of the output jax
     */
    public static NAME: string = 'JSON';
    ...
    /**
     *  The default styles for JSON
     */
    public static commonStyles: CssStyleList = {
        'mjx-container[jax="JSON"]': {
            direction: 'ltr'
        },
        'mjx-container[jax="JSON"] > svg': {
            overflow: 'visible',
            'min-height': '1px',
            'min-width': '1px'
        },
        'mjx-container[jax="JSON"] > svg a': {
            fill: 'blue', stroke: 'blue'
        }
    };

    /**
     * The ID for the JSON element that stores the cached font paths
     */
    public static FONTCACHEID = 'MJX-JSON-global-cache';

    /**
     * The ID for the stylesheet element for the styles for the JSON output
     */
    public static STYLESHEETID = 'MJX-JSON-styles';

    /**
     * Stores the CHTMLWrapper factory
     */
    public factory: JSONWrapperFactory<N, T, D>;
    ...
    /**
     * The JSON stylesheet, once it is constructed
     */
    ...
    constructor(options: OptionList = null) {
        super(options, JSONWrapperFactory as any, TeXFont);
    ...
    public styleSheet(html: MathDocument<N, T, D>) {
        ...
        this.adaptor.setAttribute(sheet, 'id', JSONOutput.STYLESHEETID);
        return sheet;
    }
    ...
    public pageElements(html: MathDocument<N, T, D>) {
        if (this.options.fontCache === 'global' && !this.findCache(html)) {
            return this.svg('svg', {id: JSONOutput.FONTCACHEID, style: {display: 'none'}}, [this.fontCache.getCache()]);
        }
        return null as N;
    }
     ...
     * @return {N}                      The newly created node in the JSON namespace
     */
    ...
    public getFullVariant() {
        return this.font.getFullVariant()
    }
    ...
    protected findCache(html: MathDocument<N, T, D>): boolean {
        ...
        for (let i = svgs.length - 1; i >= 0; i--) {
            if (this.adaptor.getAttribute(svgs[i], 'id') === JSONOutput.FONTCACHEID) {
                return true;
            }
        }
        return false;
    ...
    /**
     * @param {MmlNode} math  The MML node whose JSON is to be produced
     * @param {N} parent      The HTML node to contain the JSON
     */
    protected processMath(math: MmlNode, parent: N) {
        ...
        //
        //  Get the wrapped math element and the JSON container
        //  Then typeset the math into the JSON
        //
        const wrapper = this.factory.wrap(math);
        const [svg, g] = this.createRoot(wrapper);
        this.typesetJSON(wrapper, svg, g);
        //
    ...
    /**
     * @param {JSONWrapper} wrapper   The wrapped math to process
     * @return {[N, N]}              The svg and g nodes for the math
     */
    protected createRoot(wrapper: JSONWrapper<N, T, D>): [N, N] {
     ...
     *
     * @param {JSONWrapper} wrapper   The wrapped math to typeset
     * @param {N} svg                The main svg element for the typeet math
     * @param {N} g                  The group in which the math is typeset
     */
    protected typesetJSON(wrapper: JSONWrapper<N, T, D>, svg: N, g: N) {
        const adaptor = this.adaptor;
        //
        //  Typeset the math and add minWidth (from mtables), or set the alignment and indentation
        //    of the finalized expression
