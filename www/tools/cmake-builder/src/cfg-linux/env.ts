...
const GCC_HOST_ROOT = path.resolve(os.homedir(), 'shared/gcc');

const LLVM_INSTALL_DIR = path.resolve(os.homedir(), 'shared/llvm');

const CMAKE_BIN_DIR = path.resolve(os.homedir(), 'shared/cmake/bin');

const CUDAToolkit_ROOT = path.resolve('/usr/local/cuda-12');

const QT_HOST_ROOT = path.resolve(os.homedir(), 'shared/Qt/6.5.3/gcc_64');
