...
#define CURRENT (*TOP.value)
    ...
    JsonReader::JsonReader(const char *json) : mDocument(), mStack(), mError(false) {
        ...
        if (DOCUMENT->HasParseError()) {
            ...
            STACK->emplace(DOCUMENT, JsonReaderStackItem::BeforeStart);
        }
    }
        ...
        if (!mError) {
            if (CURRENT.IsObject() && TOP.state == JsonReaderStackItem::Started) {
                ...
                if (memberItr != CURRENT.MemberEnd())
                    STACK->emplace(&memberItr->value, JsonReaderStackItem::BeforeStart);
                else {
                    mError = true;
                    SPDLOG_ERROR("error");
                }
                ...
                mError = true;
                SPDLOG_ERROR("error state={}", int{TOP.state});
            }
        }
        return *this;
    }
        ...
        if (!mError) {
            if (CURRENT.IsArray() && TOP.state == JsonReaderStackItem::BeforeStart) {
                ...
                if (!CURRENT.Empty()) {
                    ...
                    STACK->emplace(value, JsonReaderStackItem::BeforeStart);
                } else
    ...
    JsonReader &JsonReader::TraverseAllMembers(
